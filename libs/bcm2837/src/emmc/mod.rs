...
pub struct Emmc<'a, TIME: drivers::time::Time> {
    emmc: pac::EMMC,
    time: &'a TIME,
    ...
    TIME: drivers::time::Time,
{
    ...
    fn wait_for_interrupt<F>(&self, timeout: Duration, f: F) -> Result
    where
        F: Fn(bcm2837_pac::emmc::interrupt::R) -> bcm2837_pac::generic::BitReader<bool>,
    {
...
pub enum Error {
    ...
    AppCmdNotAccepted,
    Status(CardStatus),
    Time(drivers::time::SpinError),
}

impl From<drivers::time::SpinError> for Error {
    fn from(e: drivers::time::SpinError) -> Self {
        Self::Time(e)
    }
}
