pub fn find_magic_header(output: &mut impl core::fmt::Write) {
    for addr in (crate::sys::kernel_end()..crate::config::MAX_MEMORY as usize).step_by(8) {
        let val = unsafe { core::ptr::read_volatile(addr as usize as *const u64) };
        if val != 0 {
            let _ = writeln!(output, "0x{:016X}: 0x{:016X}", addr, val);
        }
    }
}
