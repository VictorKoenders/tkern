...
typedef struct _IDENTIFY_DEVICE_DATA
{
    ...
    USHORT NumCylinders;          // 1
    USHORT SpecificConfiguration; // 2
    USHORT NumHeads;              // 3
    USHORT Retired1[2];           // 4 + 5
    USHORT NumSectorsPerTrack;    // 6
    USHORT VendorUnique1[3];      // 7 + 8 + 9
    UCHAR SerialNumber[20];       // 10-19 (note: uchar is half a ushort, so only 10 ushorts are needed for this field)
    USHORT Retired2[2];           // 20 + 21
    USHORT Obsolete1;             // 22
    UCHAR FirmwareRevision[8];    // 23-26 (note: uchar)
    UCHAR ModelNumber[40];        // 27-37 (note: uchar)
    UCHAR MaximumBlockTransfer;   // 38, shares the position with VendorUnique2
    UCHAR VendorUnique2;          // 38, shares the position with MaximumBlockTransfer
    struct
    {
    ...
    USHORT ObsoleteWords51[2];                   // 42 + 43
    USHORT TranslationFieldsValid : 3;           // 44
    USHORT Reserved3 : 5;                        // 44
    USHORT FreeFallControlSensitivity : 8;       // 44
    USHORT NumberOfCurrentCylinders;             // 45
    USHORT NumberOfCurrentHeads;                 // 46
    USHORT CurrentSectorsPerTrack;               // 47
    ULONG CurrentSectorCapacity;                 // 48-51
    UCHAR CurrentMultiSectorSetting;             // 52
    UCHAR MultiSectorSettingValid : 1;           // 52
    UCHAR ReservedByte59 : 3;                    // 52
    UCHAR SanitizeFeatureSupported : 1;          // 52
    UCHAR CryptoScrambleExtCommandSupported : 1; // 52
    UCHAR OverwriteExtCommandSupported : 1;      // 52
    UCHAR BlockEraseExtCommandSupported : 1;     // 52
    ULONG UserAddressableSectors;                // 53-56
    USHORT ObsoleteWord62;                       // 57
    USHORT MultiWordDMASupport : 8;              // 58
    USHORT MultiWordDMAActive : 8;               // 58
    USHORT AdvancedPIOModes : 8;                 // 58
    USHORT ReservedByte64 : 8;                   // 58
    USHORT MinimumMWXferCycleTime;               // 59
    USHORT RecommendedMWXferCycleTime;           // 60
    USHORT MinimumPIOCycleTime;                  // 61
    USHORT MinimumPIOCycleTimeIORDY;             // 62
    struct
    {
