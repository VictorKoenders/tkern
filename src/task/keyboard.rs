...
pub async fn print_keypresses() {
    ...
    // ScancodeStream never returns Poll::Ready(None), so this is unreachable
    unsafe {
...
impl Stream for ScancodeStream {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Option<u8>> {
        // This should never return Poll::Ready(None) or it would conflict with the unsafe code found in `print_keypresses`
